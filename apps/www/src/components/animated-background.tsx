...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    // Use Float32Array for iResolution (w, h, aspect)
    const iRes = new Float32Array(3);
    ...
    function setSize(width: number, height: number) {
      ...
      iRes[0] = gl.canvas.width;
      iRes[1] = gl.canvas.height;
      iRes[2] = gl.canvas.width / gl.canvas.height;
    }
    ...
    // When used as viewport background, size from window
    const ro = new ResizeObserver(resize);
    ro.observe(ctn);

    const onWinResize = () => {
      if (mode === "viewport") resize();
    };
  ...
  // Important: give it actual dimensions when in viewport mode.
  const base =
    mode === "viewport"
